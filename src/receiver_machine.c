				   ...
				   struct event *event, const cfdpCfdpPDU *pdu)
{
	...
	if(receiver_machine->state == WAIT_FOR_MD)
	{
		switch(event->type)
		{
			...
			{
				cfdp_core_abandoned_indication(receiver_machine->core, receiver_machine->transaction_id);
				receiver_machine_close(receiver_machine);
				break;
			}
			case E3_NOTICE_OF_CANCELLATION:
			{
				cfdp_core_finished_indication(receiver_machine->core, receiver_machine->transaction_id);
				receiver_machine_close(receiver_machine);
				break;
			}
			case E10_RECEIVED_METADATA:
			{
				cfdp_core_metadata_received_indication(receiver_machine->core, receiver_machine->transaction_id);
				if(transaction_is_file_transfer_in_progress(&receiver_machine->transaction)){
					transaction_open_temp_file(&receiver_machine->transaction);
				}
			...
			{
				assert(pdu != NULL);
				receiver_machine->condition_code = pdu->payload.u.file_directive.file_directive_pdu.u.eof_pdu.condition_code;
			...
			{
				receiver_machine->condition_code = cfdpConditionCode_cancel_request_received;
				cfdp_core_issue_request(receiver_machine->core, receiver_machine->transaction_id, E3_NOTICE_OF_CANCELLATION);
				break;
			}
		...
		switch(event->type)
		{
			...
			{
				cfdp_core_filesegment_received_indication(receiver_machine->core, receiver_machine->transaction_id);
				
				if(transaction_is_file_transfer_in_progress(&receiver_machine->transaction))
				{
					const cfdpFileDataPDU *file_data_pdu = &(pdu->payload.u.file_data.file_data_pdu);
					transaction_store_data_in_temp_file(&receiver_machine->transaction, file_data_pdu);
					if(receiver_machine->received_file_size < file_data_pdu->segment_offset + file_data_pdu->file_data.nCount)
					{
						receiver_machine->received_file_size = file_data_pdu->segment_offset + file_data_pdu->file_data.nCount;
					}
				}
				break;
			}
			case E12_RECEIVED_EOF_NO_ERROR:
			{
				cfdp_core_eof_received_indication(receiver_machine->core, receiver_machine->transaction_id);
				...
				if(transaction_is_file_transfer_in_progress(&receiver_machine->transaction))
				{
					...
					receiver_machine->delivery_code = DeliveryCode_data_complete;
					transaction_copy_temp_file_to_dest_file(&receiver_machine->transaction);
					transaction_close_temp_file(&receiver_machine->transaction);
				}
